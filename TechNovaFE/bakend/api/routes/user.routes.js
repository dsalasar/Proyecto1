...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!email || !password) return res.status(400).json({ msg: 'Debe llenar los campos requeridos' });
...
// RF1.4  Cambiar contraseña 
// POST http://localhost:3000/api/users/:id/pasword
router.post('/:id/password', async (req, res) => {
  try {
    ...
    if (!newPassword || newPassword.length < 6) {
      return res.status(400).json({ msg: 'La nueva contraseña no cumple con los requisitos' });
    }
    ...
    console.error(e);
    return res.status(500).json({ msg: 'Error al cambiar la contraseña', error: e.message });
  }
});
